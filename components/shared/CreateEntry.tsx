...
const CreateEntry: React.FC<CreateEntryProps> = ({ titleId, onEntryCreated }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (roleId === 1) {
          successNote(t('rookie_entry_submitted', 'Entry submitted! It will be reviewed by moderation.'));
        } else {
          successNote(t('entry_created', 'Entry posted successfully!'));
        }
        formik.resetForm();
        if (onEntryCreated) {
          onEntryCreated();
  ...
  return (
    <div className="my-6 p-5 bg-white dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-800 shadow-sm transition-colors duration-200">
      <h3 className="text-sm font-bold text-gray-800 dark:text-slate-200 mb-3">
        {t('new_entry', 'Write an Entry')}
      </h3>
      ...
      {isAuthenticated ? (
        <form onSubmit={formik.handleSubmit} className="space-y-3">
          ...
          <div className="flex justify-end">
            ...
            >
              {isSubmitting ? t('posting', 'Posting...') : t('post_entry', 'Submit Entry')}
            </button>
          </div>
        </form>
      ) : (
        <div className="py-6 text-center space-y-3 bg-gray-50 dark:bg-slate-800/60 rounded-lg">
          <p className="text-xs text-gray-600 dark:text-slate-400">
            {t('login_alert', 'Please sign in or create an account to post an entry.')}
          </p>
